...
关键内容不依赖LaTeX或特定数学渲染插件。

- 变量、字段、寄存器和代码表达式使用行内代码，例如 `base`、`length`、`sizeof(int)`。
- 地址计算、复杂度推导和二进制运算使用 `text` 代码块。
- 使用 `*`、`/`、`<=`、`>=`、`Theta(n)`、`log2(n)` 等纯文本记法。
- 推导较长时逐行写出，不使用美元符号定界的公式语法或其他仅部分查看器支持的公式扩展。
